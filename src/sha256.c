...
static uint32_t romx_rotate_right(uint32_t value, unsigned int count)
{
    return (value >> count) | (value << (32U - count));
}

static void romx_sha256_transform(romx_sha256_context_t *context, const uint8_t *data)
{
    ...
    for (index = 0U; index < 16U; ++index) {
        words[index] = ((uint32_t)data[index * 4U] << 24)
            | ((uint32_t)data[index * 4U + 1U] << 16)
            | ((uint32_t)data[index * 4U + 2U] << 8)
            | (uint32_t)data[index * 4U + 3U];
    }
    for (index = 16U; index < 64U; ++index) {
        const uint32_t s0 = romx_rotate_right(words[index - 15U], 7U)
            ^ romx_rotate_right(words[index - 15U], 18U)
            ^ (words[index - 15U] >> 3U);
        const uint32_t s1 = romx_rotate_right(words[index - 2U], 17U)
            ^ romx_rotate_right(words[index - 2U], 19U)
            ^ (words[index - 2U] >> 10U);
    ...
    for (index = 0U; index < 64U; ++index) {
        const uint32_t s1 = romx_rotate_right(e, 6U) ^ romx_rotate_right(e, 11U)
            ^ romx_rotate_right(e, 25U);
        const uint32_t choice = (e & f) ^ ((~e) & g);
        const uint32_t temp1 = h + s1 + choice + constants[index] + words[index];
        const uint32_t s0 = romx_rotate_right(a, 2U) ^ romx_rotate_right(a, 13U)
            ^ romx_rotate_right(a, 22U);
...
void romx_sha256_update(romx_sha256_context_t *context, const uint8_t *data, size_t size)
{
    context->total_size += (uint64_t)size;
    while (size != 0U) {
        size_t count = sizeof(context->block) - context->block_size;
        ...
        if (context->block_size == sizeof(context->block)) {
            romx_sha256_transform(context, context->block);
            context->block_size = 0U;
        }
    }
}

void romx_sha256_finish(romx_sha256_context_t *context, uint8_t digest[32])
{
    const uint64_t bit_size = context->total_size * UINT64_C(8);
    size_t index;
